...
echo "==== add delivery addition cart item ===="
...
echo
echo "==== checkout delivery addition ===="
curl -sS "$API_BASE/customer/checkout/address" \
  -H "Authorization: Bearer $CUSTOMER_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"address":"Selchower Str. 10, 12049 Berlin","label":"Smoke test delivery location","google_maps_link":"https://maps.google.com/?q=Selchower%20Str.%2010%2C%2012049%20Berlin","notes":"V2 smoke delivery addition"}' \
...
echo
echo "==== approve pending delivery group ===="
curl -sS "$API_BASE/admin/customer-app-orders/$ORDER_ID/groups/$GROUP_ID/approve" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"note":"V2 smoke approve delivery addition"}' \
