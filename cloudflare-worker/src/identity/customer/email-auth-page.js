...
<h1>Customer Shop</h1><p class="muted">Continue with your email. We will sign you in or create your customer account after verification.</p>
<p id="status" aria-live="polite"></p>
<form id="email-form"><label for="email">Email address</label><input id="email" name="email" type="email" autocomplete="email" required><button type="submit">Email me a verification code</button></form>
...
const showProfile=data=>{$("email-form").classList.add("hidden");$("code-form").classList.add("hidden");$("confirm").classList.add("hidden");$("profile").classList.remove("hidden");$("profile-name").textContent=data.customer?.full_name||"Customer";$("profile-email").textContent=data.customer?.email||"Verified email";message("Your email is verified and you are signed in.","success")};
